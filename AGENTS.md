...
1. **Backend** — FastAPI + Alembic + SQLite (данные, роутинг задач для воркеров/фронтенда)
...
**Почему SQLite:** Минимальная стоимость эксплуатации — не нужен отдельный сервис БД, бэкапы простым копированием файла, достаточно для 1 воркера на запись.
